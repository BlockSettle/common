...
bool ZmqServerConnection::BindConnection(const std::string& host
   , const std::string& port, ServerConnectionListener* listener)
{
   assert(context_ != nullptr);
   ...
   int lingerPeriod = 0;
   int result = zmq_setsockopt(tempMonSocket.get(), ZMQ_LINGER, &lingerPeriod
      , sizeof(lingerPeriod));
   if (result != 0) {
   ...
   result = zmq_socket_monitor(tempDataSocket.get()
      , monitorConnectionName_.c_str(), ZMQ_EVENT_ALL);
   if (result != 0) {
   ...
   if (result != 0) {
      logger_->error("[{}] failed to bind socket to {} : {}", __func__, endpoint
...
void ZmqServerConnection::listenFunction()
{
   ...
   while(true) {
      ...
      if (poll_items[ZmqServerConnection::ControlSocketIndex].revents & ZMQ_POLLIN) {
         ...
         int recv_result = zmq_msg_recv(&command
            , poll_items[ZmqServerConnection::ControlSocketIndex].socket
            , ZMQ_DONTWAIT);
         if (recv_result == -1) {
...
void ZmqServerConnection::stopServer()
{
...

   {
      ...
      result = zmq_send(threadMasterSocket_.get(), static_cast<void*>(&command)
         , sizeof(command), 0);
   }
...
void ZmqServerConnection::notifyListenerOnData(const std::string& clientId
...
bool ZmqServerConnection::QueueDataToSend(const std::string& clientId
   , const std::string& data, const SendResultCb &cb, bool sendMore)
{
   {
...
void ZmqServerConnection::SendDataToDataSocket()
{
   ...
   for (const auto &dataPacket : pendingData) {
      ...
      result = zmq_send(dataSocket_.get(), dataPacket.data.data()
         , dataPacket.data.size(), (dataPacket.sendMore ? ZMQ_SNDMORE : 0));
...
bool ZmqServerConnection::ConfigDataSocket(const ZmqContext::sock_ptr &dataSocket)
{
   ...
   if (zmq_setsockopt(dataSocket.get(), ZMQ_IMMEDIATE, &immediate, sizeof(immediate)) != 0) {
      logger_->error("[{}] {} failed to set immediate flag: {}", connectionName_
         , zmq_strerror(zmq_errno()), __func__);
      return false;
   }

   if (!identity_.empty()) {
      if (zmq_setsockopt(dataSocket.get(), ZMQ_IDENTITY, identity_.c_str()
         , identity_.size()) != 0) {
